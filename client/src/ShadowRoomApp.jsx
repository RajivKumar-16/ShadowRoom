...
export function ShadowRoomApp() {
  ...
  const [downloadProgressByFile, setDownloadProgressByFile] = useState({});
  ...
  const handleJoinRoom = async (e) => {
    ...
    try {
      ...
      window.history.replaceState(null, "", `/?roomCode=${roomCodeInput.trim()}`);
  ...
  const handleFileUpload = async (file) => {
    ...
    // 1) close upload modal immediately when user confirms sending
    setFileModalOpen(false);

    if (file.size > MAX_UPLOAD_BYTES) {
      setError("File is too large. Maximum upload is 2GB.");
      return;
    }
    ...
    try {
      ...
      const uploadAborted = err?.name === "CanceledError" || axios.isCancel?.(err);
      setUploadProgress(null);
      uploadAbortControllerRef.current = null;
      if (uploadAborted) {
        showToast("Info", "File upload canceled.", "info");
      } else {
        showToast(
          "Error",
  ...
  const renderFileMessage = (m) => {
    const fullUrl = m.fileUrl?.startsWith("http") ? m.fileUrl : `${API_BASE}${m.fileUrl}`;
      ...
      <div className="file-bubble-container">
        {isImage ? (
          ...
          <div className="file-bubble-doc">
            ...
            <div className="file-bubble-doc-info">
              ...
              <div className="file-bubble-doc-size">{sizeLabel}</div>
            </div>
          </div>
        )}
        ...
        <div className="file-bubble-actions">
          ...
          <button
            ...
            onClick={async () => {
              const key = m.fileId || m.fileName || "download";
              setDownloadProgressByFile((prev) => ({ ...prev, [key]: 0 }));
              try {
                ...
                while (true) {
                  ...
                  if (contentLength > 0) {
                    ...
                    setDownloadProgressByFile((prev) => ({ ...prev, [key]: pct }));
                  } else {
                    // fallback spinner style progress
                    setDownloadProgressByFile((prev) => ({ ...prev, [key]: Math.min((prev[key] || 0) + 5, 90) }));
                  }
                }
                ...
                anchor.href = href;
                anchor.download = m.fileName || "download";
                document.body.appendChild(anchor);
                anchor.click();
                anchor.remove();
                setTimeout(() => URL.revokeObjectURL(href), 1000);

                setDownloadProgressByFile((prev) => ({ ...prev, [key]: 100 }));
                setTimeout(() => {
                  setDownloadProgressByFile((prev) => {
                    const next = { ...prev };
                    delete next[key];
                    return next;
                  });
                ...
                showToast("Error", "Download failed. Please try again.", "error");
              }
            }}
          >
        ...
        {downloadProgressByFile[m.fileId || m.fileName] != null && (
          <div className="download-progress-bar">
            <div
              className="download-progress-fill"
              style={{ width: `${downloadProgressByFile[m.fileId || m.fileName]}%` }}
            />
          </div>
        )}
      </div>
          ...
          <div className="modal-body">
            ...
            >
              ...
              <p className="upload-hint">
                PDF, DOCX, TXT, Images, ZIP (Max 2GB)
              </p>
            </div>
            <input
