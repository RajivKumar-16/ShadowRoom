...
if (typeof Peer !== "function") {
  throw new Error(
    "simple-peer: constructor not found. Try clearing node_modules/.vite and restarting the dev server."
  );
}
...
export function createWebRtcSession({
  ...
  const waitForChannelOpen = async () => {
    ...
    if (!isChannelOpen()) {
      throw new Error("Data channel not ready. Wait a moment and try again.");
    }
  };
  ...
  const sendBinary = async (ab) => {
    ...
    if (aesKey) {
      try {
        const encrypted = await encryptBuffer(
          aesKey,
          ab instanceof ArrayBuffer ? ab : ab.buffer || ab
        );
        peer.send(encrypted);
        return;
      } catch {
  ...
  const saveIncomingToDisk = async (id) => {
    ...
    if (!canUseFileSystemAccessApi()) {
      throw new Error("Save-to-device not supported in this browser.");
    }
  ...
  const setupPeer = () => {
    ...
    peer.on("error", (err) => {
      ...
      onTransferEvent?.({
        type: "system",
        message: err?.message || "WebRTC error.",
      });
    });
      ...
      if (str != null) {
        ...
        if (msg.t === "file-meta") {
          ...
          const shouldAutoStream =
            canSaveToDisk && msg.size >= 200 * 1024 * 1024;
          incoming.set(msg.id, {
            meta: msg,
            receivedBytes: 0,
            chunks: [],
